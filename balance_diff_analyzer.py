...
def parse_balances(balance_text):
    ...
    for line in lines:
        ...
        if line.startswith('- amount:') or line.startswith('amount:'):
            ...
            balances[denom] = int(amount)
    
    return balances

def calculate_differences(balances1, balances2):
    """Calculate differences between two balance sets."""
    ...
    for denom in all_denoms:
        ...
        differences[denom] = {
            'first_observation': amount1,
            'second_observation': amount2,
            'difference': diff,
            'percentage_change': (diff / amount1 * 100) if amount1 != 0 else float('inf')
        }
...
- amount: "11272531150"
  denom: uelys"""
    ...
    differences = calculate_differences(balances1, balances2)
    
    # Print results
    print("BALANCE DIFFERENCE ANALYSIS")
    print("=" * 80)
    print(f"{'Denom':<70} {'First':<15} {'Second':<15} {'Difference':<15} {'% Change':<10}")
    print("-" * 80)
    
    for denom, data in differences.items():
        first = data['first_observation']
        second = data['second_observation']
        ...
        # Truncate long denom names for display
        display_denom = denom[:67] + "..." if len(denom) > 70 else denom
        
        print(f"{display_denom:<70} {first:<15} {second:<15} {diff:<15} {pct_str:<10}")
    
    print("\n" + "=" * 80)
    print("SUMMARY:")
    
    # Count increases, decreases, and unchanged
    increases = sum(1 for data in differences.values() if data['difference'] > 0)
    decreases = sum(1 for data in differences.values() if data['difference'] < 0)
    unchanged = sum(1 for data in differences.values() if data['difference'] == 0)
    
    print(f"Tokens with increases: {increases}")
    print(f"Tokens with decreases: {decreases}")
    print(f"Tokens unchanged: {unchanged}")
    
    # Find largest changes
    largest_increase = max(differences.items(), key=lambda x: x[1]['difference'])
    largest_decrease = min(differences.items(), key=lambda x: x[1]['difference'])
    
    print(f"\nLargest increase: {largest_increase[0]}")
    print(f"  Change: {largest_increase[1]['difference']:,}")
    
    print(f"\nLargest decrease: {largest_decrease[0]}")
    print(f"  Change: {largest_decrease[1]['difference']:,}")
