...
func (min MinCommissionDecorator) AnteHandle(
	...
	validMsg := func(m sdk.Msg) error {
		...
		case *stakingtypes.MsgCreateValidator:
			...
			// commission set below the param set
			...
			// if max voting power exceeds the param set
			...
			// if min self delegation is not matching to the param set
			if !msg.MinSelfDelegation.Equal(params.MinSelfDelegation) {
				return sdkerrors.Wrapf(
					sdkerrors.ErrInvalidRequest,
					"Min delegation has be the same as %s", params.MinSelfDelegation)
			}
