...
elysd tx bank send treasury elys1r8nfpk0t4g6r7542g99s8ymxwzpsg57alx7gue 1000000000000uelys --from=treasury --keyring-backend=test --chain-id=elystestnet-1 --yes 
elysd tx staking delegate elysvaloper12tzylat4udvjj56uuhu3vj2n4vgp7cf9pwcqcs 100000000000uelys --from=user1 --keyring-backend=test --chain-id=elystestnet-1 --yes --gas=1000000
...
elysd tx incentive withdraw-rewards --from=user1 --keyring-backend=test --chain-id=elystestnet-1 --yes 
elysd tx commitment commit-claimed-rewards 342464 ueden --from=user1 --keyring-backend=test --chain-id=elystestnet-1 --yes --gas=1000000

# create vesting from Eden to Elys
elysd tx commitment vest 1284240 ueden --from=user1 --keyring-backend=test --chain-id=elystestnet-1 --yes --gas=1000000
elysd query commitment show-commitments elys1r8nfpk0t4g6r7542g99s8ymxwzpsg57alx7gue

# create liquidity pool
elysd tx amm create-pool 10uatom,10uusdc 100000000000uatom,10000000000uusdc --swap-fee=0.00 --exit-fee=0.00 --use-oracle=false --from=treasury --keyring-backend=test --chain-id=elystestnet-1 --yes --gas=1000000
elysd tx stablestake bond 19800000000 --from=treasury --keyring-backend=test --chain-id=elystestnet-1 --yes --gas=1000000
...
elysd tx commitment commit-claimed-rewards 173430717 ueden --from=treasury --keyring-backend=test --chain-id=elystestnet-1 --yes --gas=1000000
...
elysd tx staking delegate elysvaloper12tzylat4udvjj56uuhu3vj2n4vgp7cf9pwcqcs 100000000000uelys --from=treasury --keyring-backend=test --chain-id=elystestnet-1 --yes --gas=1000000
...
elysd tx masterchef claim-rewards --pool-ids=32767 --from=treasury --keyring-backend=test --chain-id=elystestnet-1 --yes --gas=1000000
