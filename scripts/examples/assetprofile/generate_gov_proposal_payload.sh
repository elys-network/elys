...
generate_json() {
    ...
    cat <<EOF
    {
      ...
      "decimals": "$DECIMALS",
      "denom": "ibc/$DENOM",
      "path": "transfer/channel-$CHANNEL_ID",
      "ibc_channel_id": "channel-$CHANNEL_ID",
      "ibc_counterparty_channel_id": "channel-$EXTERNAL_CHANNEL_ID",
      "display_name": "$NAME",
      "display_symbol": "$BASE_DENOM",
      "network": "",
      "address": "",
      "external_symbol": "$BASE_DENOM",
      "transfer_limit": "",
      "permissions": [],
      "unit_denom": "$BASE_DENOM",
...
# Main script
if [ "$#" -lt 5 ]; then
    echo "Usage: $0 <base_denom1> <name1> <decimals1> <channel_id1> <external_channel_id1> [<base_denom2> <name2> <decimals2> <channel_id2> <external_channel_id2> ...]"
    exit 1
fi
