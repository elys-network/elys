...
elysd tx amm create-pool 10uatom,10uusdc 20000000000uatom,20000000000uusdc --swap-fee=0.01 --exit-fee=0.00 --use-oracle=false --from=treasury --keyring-backend=test --chain-id=elystestnet-1 --yes --gas=1000000
...
# swap
elysd tx amm swap-exact-amount-in 10000000uatom 1 1 uusdc --from=treasury --keyring-backend=test --chain-id=elystestnet-1 --yes --gas=1000000
...
elysd tx masterchef add-external-incentive uinc 1 80 1080 1000000 --from=treasury --keyring-backend=test --chain-id=elystestnet-1 --yes --gas=1000000
