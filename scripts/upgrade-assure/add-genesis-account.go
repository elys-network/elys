...
func addGenesisAccount(cmdPath, address, balance, homePath string) {
	...
	if err := exec.Command(cmdPath, args...).Run(); err != nil {
		log.Fatalf(ColorRed+"Command execution failed: %v", err) // nolint: goconst
	}

	// If execution reaches here, the command was successful
	log.Printf(ColorYellow+"add genesis account with address %s, balance: %s and home path %s successfully", address, balance, homePath)
}
