...
# echo "Tx: Create Pool"
# $BINARY tx amm create-pool 10uatom,10uusdc 1000000000000uatom,1000000000000uusdc uusdc --use-oracle=true --from=$MY_VALIDATOR_ADDRESS --keyring-backend=test --chain-id=elys --gas=1000000
# query_tx "create pool"

# echo "Tx: stablestake Bond, Put funds on stablestake"
# $BINARY tx stablestake bond 1000000000000 --from=$MY_VALIDATOR_ADDRESS --keyring-backend=test --chain-id=elys --gas=1000000
...
# $BINARY tx gov submit-proposal ${current_dir}/scripts/test_scripts/leveragelp/proposal_add_pool.json --from=$MY_VALIDATOR_ADDRESS --keyring-backend=test --chain-id=elys --gas=1000000
...
# $BINARY tx leveragelp open 2.0 uusdc 1000000 1 0.0 --from=$MY_VALIDATOR_ADDRESS --chain-id elys --gas 5000000 --keyring-backend="test"
...
$BINARY tx leveragelp close 1 1999747632999781945 --from=$MY_VALIDATOR_ADDRESS --chain-id elys --gas 5000000 --keyring-backend="test"
...
# $BINARY tx gov submit-proposal ${current_dir}/scripts/test_scripts/leveragelp/proposal_remove_pool.json --from=$MY_VALIDATOR_ADDRESS --keyring-backend=test --chain-id=elys --gas=1000000
# query_tx "create gov proposal"
# sh ${current_dir}/scripts/test_scripts/vote.sh
