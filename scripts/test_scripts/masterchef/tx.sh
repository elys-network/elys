...
# $BINARY tx gov submit-proposal ${current_dir}/scripts/test_scripts/masterchef/add_external_reward_denom.json --from=$MY_VALIDATOR_ADDRESS --keyring-backend=test --chain-id=elys --gas=1000000
# query_tx "create gov proposal"
# sh ${current_dir}/scripts/test_scripts/vote.sh

# sleep 80s
$BINARY tx masterchef add-external-incentive uatom 1 2270 240000 10 --from=$MY_VALIDATOR_ADDRESS --keyring-backend=test --chain-id=elys --gas=1000000
...
# $BINARY tx gov submit-proposal ${current_dir}/scripts/test_scripts/masterchef/toggle_pool_eden_rewards.json --from=$MY_VALIDATOR_ADDRESS --keyring-backend=test --chain-id=elys --gas=1000000
# query_tx "create gov proposal"
# sh ${current_dir}/scripts/test_scripts/vote.sh

# echo "Tx: Create and vote on gov Proposal for update pool multipliers "
# $BINARY tx gov submit-proposal ${current_dir}/scripts/test_scripts/masterchef/update_pool_multipliers.json --from=$MY_VALIDATOR_ADDRESS --keyring-backend=test --chain-id=elys --gas=1000000
# query_tx "create gov proposal"
# sh ${current_dir}/scripts/test_scripts/vote.sh


# echo "Tx: Create and vote on gov Proposal for update params "
# $BINARY tx gov submit-proposal ${current_dir}/scripts/test_scripts/masterchef/update_params.json --from=$MY_VALIDATOR_ADDRESS --keyring-backend=test --chain-id=elys --gas=1000000
# query_tx "create gov proposal"
# sh ${current_dir}/scripts/test_scripts/vote.sh