...
func TestClaimReward(t *testing.T) {
	...
	err := app.CommitmentKeeper.RecordClaimReward(ctx, creator, denom, sdk.NewInt(30), types.WITHDRAW_MODE_ALL)
	...
	err = app.CommitmentKeeper.RecordClaimReward(ctx, creator, denom, sdk.NewInt(70), types.WITHDRAW_MODE_ALL)
	require.Error(t, err)
}
