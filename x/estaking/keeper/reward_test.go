...
// Add a governor for Eden and EdenB to be non zero
...
//				suite.Require().Equal(len(commitments), 2)
//
...
//				suite.Require().Equal(balances.AmountOf(ptypes.Eden).String(), "0") // No Eden Rewards without a governor
//				suite.Require().Equal(balances.AmountOf(ptypes.EdenB).String(), "0")
//
//				totalBonded, err = suite.app.EstakingKeeper.TotalBondedTokens(suite.ctx)
//				suite.Require().NoError(err)
//				suite.Require().Equal(totalBonded.String(), "100001000000")
