	...
	testCases := []struct {
		...
		// --- Success Cases ---
		{
			...
			setupSpecific: func() {
				...
				sellOrder := types.NewOrder(MarketId, types.OrderType_ORDER_TYPE_LIMIT_BUY, math.LegacyNewDec(99), uint64(suite.ctx.BlockHeight()), counterpartySubAccount.GetOwnerAccAddress(), math.LegacyNewDec(10), math.LegacyZeroDec(), MarketId)
				suite.app.ClobKeeper.SetOrder(suite.ctx, sellOrder)
			},
		...
		},
		{
			...
			setupSpecific: func() {
				...
				buyOrder := types.NewOrder(MarketId, types.OrderType_ORDER_TYPE_LIMIT_SELL, math.LegacyNewDec(101), uint64(suite.ctx.BlockHeight()), counterpartySubAccount.GetOwnerAccAddress(), math.LegacyNewDec(10), math.LegacyZeroDec(), MarketId)
				suite.app.ClobKeeper.SetOrder(suite.ctx, buyOrder)
			},
			expectedRatio: math.LegacyOneDec(),
			expectADL:     false,
			postCheck: func(originalPerp types.Perpetual) {
		...
		},
		{
			...
			setupSpecific: func() {
				...
				sellOrder := types.NewOrder(MarketId, types.OrderType_ORDER_TYPE_LIMIT_BUY, math.LegacyNewDec(99), uint64(suite.ctx.BlockHeight()), counterpartySubAccount.GetOwnerAccAddress(), math.LegacyNewDec(5), math.LegacyZeroDec(), MarketId) // Only 5 available
				suite.app.ClobKeeper.SetOrder(suite.ctx, sellOrder)
			},
		...
		},
		{
			...
			setupSpecific: func() {
				buyOrder := types.NewOrder(MarketId, types.OrderType_ORDER_TYPE_LIMIT_SELL, math.LegacyNewDec(101), uint64(suite.ctx.BlockHeight()), counterpartySubAccount.GetOwnerAccAddress(), math.LegacyNewDec(7), math.LegacyZeroDec(), MarketId) // Only 7 available
				suite.app.ClobKeeper.SetOrder(suite.ctx, buyOrder)
			},
		...
		// --- Error Cases ---
		{
			...
			setupSpecific: func() {
				...
				sellOrder := types.NewOrder(MarketId, types.OrderType_ORDER_TYPE_LIMIT_BUY, math.LegacyNewDec(1), uint64(suite.ctx.BlockHeight()), counterpartySubAccount.GetOwnerAccAddress(), math.LegacyNewDec(10), math.LegacyZeroDec(), MarketId)
				suite.app.ClobKeeper.SetOrder(suite.ctx, sellOrder)
...
func (suite *KeeperTestSuite) TestForcedLiquidation() { // Using KeeperTestSuite
	...
	testCases := []struct {
		...
		// --- C. Partial Fill, ADL Set for Remainder ---
		{
			...
			orderBookSetup: func() {
				suite.app.ClobKeeper.SetOrder(suite.ctx, types.NewOrder(MarketId, types.OrderType_ORDER_TYPE_LIMIT_BUY, math.LegacyNewDec(94), uint64(suite.ctx.BlockHeight()), counterpartySubAccount.GetOwnerAccAddress(), math.LegacyNewDec(5), math.LegacyZeroDec(), MarketId))
			},
			preSetup: func() {
				suite.SetPrice([]string{"ATOM"}, []math.LegacyDec{math.LegacyNewDec(94)})
			},
		...
		// --- D. IF Insufficient During MarketLiquidation's Settlement ---
		{
			...
			orderBookSetup: func() {
				suite.app.ClobKeeper.SetOrder(suite.ctx, types.NewOrder(MarketId, types.OrderType_ORDER_TYPE_LIMIT_BUY, math.LegacyNewDec(1), uint64(suite.ctx.BlockHeight()), counterpartySubAccount.GetOwnerAccAddress(), math.LegacyNewDec(10), math.LegacyZeroDec(), MarketId))
			},
			preSetup: func() {
		...
		// --- E. Market Account is empty for trader's refund, should not happen ideally ---
		{
			...
			orderBookSetup: func() { // Full fill
				suite.app.ClobKeeper.SetOrder(suite.ctx, types.NewOrder(MarketId, types.OrderType_ORDER_TYPE_LIMIT_SELL, math.LegacyNewDec(105), uint64(suite.ctx.BlockHeight()), counterpartySubAccount.GetOwnerAccAddress(), math.LegacyNewDec(10), math.LegacyZeroDec(), MarketId))
			},
			preSetup: func() {
