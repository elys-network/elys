...
// Update rewards APR for vaults
// func (k Keeper) UpdateVaultAPR(ctx sdk.Context, totalBlocksPerYear int64, edenDenomPrice osmomath.BigDec) {
...
// 		tvl, err := k.VaultUsdValue(ctx, vault.Id)
// 		if err != nil {
// 			k.Logger(ctx).Error("error getting vault value", "error", err)
// 			continue
// 		}
...
// 		firstAccum := k.FirstPoolRewardsAccum(ctx, vault.Id)
// 		lastAccum := k.LastPoolRewardsAccum(ctx, vault.Id)
// 		if lastAccum.Timestamp == 0 {
// 			continue
// 		}

// 		if firstAccum.Timestamp == lastAccum.Timestamp {
// 			poolInfo.DexApr = lastAccum.EdenReward.
