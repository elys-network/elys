...
func (k Keeper) HandleOpenEstimation(ctx sdk.Context, req *types.QueryOpenEstimationRequest) (*types.QueryOpenEstimationResponse, error) {
	...
	if req.Position == types.Position_LONG {
		...
		if mtp.CollateralAsset == baseCurrency {
			...
			custodyAmount, slippage, weightBreakingFee, swapFees, takerFees, err = k.EstimateSwapGivenIn(ctx, leveragedAmtTokenIn, mtp.CustodyAsset, ammPool, req.Address, false)
			if err != nil {
				return nil, err
			}
			if useLimitPrice {
		...
		if mtp.CollateralAsset != baseCurrency {
			...
			liabilities, slippage, weightBreakingFee, swapFees, takerFees, err = k.EstimateSwapGivenOut(ctx, sdk.NewCoin(req.Collateral.Denom, amountIn), baseCurrency, ammPool, req.Address, false)
			if err != nil {
				return nil, err
			}
			if useLimitPrice {
	...
	if req.Position == types.Position_SHORT {
		...
		liabilities, slippage, weightBreakingFee, swapFees, takerFees, err = k.EstimateSwapGivenOut(ctx, tokenOut, mtp.LiabilitiesAsset, ammPool, mtp.Address, false)
		if err != nil {
			return nil, err
		}
		if useLimitPrice {
