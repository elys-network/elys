...
func TestBorrowRateGet(t *testing.T) {
	...
	res := keeper.GetBorrowInterestRate(ctx, uint64(ctx.BlockHeight()-2), uint64(ctx.BlockTime().Unix())-(86400*365), 1, sdk.NewDec(1000), math.LegacyOneDec())
	require.Equal(t, sdk.NewDec(9500), res) // 19 * 1000 / 2

	// 2nd case: older block
	res = keeper.GetBorrowInterestRate(ctx, uint64(ctx.BlockHeight()-8), uint64(ctx.BlockTime().Unix())-(86400*365), 1, sdk.NewDec(1000), math.LegacyOneDec())
