...
func (k Keeper) CalcNetPnLAtClosing(ctx sdk.Context, returnAmount math.Int, custodyAsset string, initialCollateral sdk.Coin, closingRatio math.LegacyDec) (netPnLInUSD math.LegacyDec) {

	closedCollateralAmount := closingRatio.MulInt(initialCollateral.Amount).TruncateInt()
	closedCollateralInUSD := k.amm.CalculateUSDValue(ctx, initialCollateral.Denom, closedCollateralAmount).Dec()
