...
func TestCloseEstimation_ExistingMTP(t *testing.T) {
	...
	var (
		...
		mtp = types.MTP{
			...
			BorrowInterestUnpaidCollateral: math.NewInt(10),
		}
		pool = types.Pool{
	...
	mockChecker.On("EstimateSwapGivenOut", ctx, sdk.NewCoin(baseCurrency, math.NewInt(9400)), mtp.CollateralAsset, ammPool).Return(math.NewInt(9400), nil).Once()
	...
	assert.Equal(t, math.LegacyZeroDec(), res.PriceImpact)
	assert.Equal(t, swapFee, res.SwapFee)
	assert.Equal(t, sdk.NewCoin(mtp.CollateralAsset, math.NewInt(9400)), res.ReturnAmount)
}
