...
func (k Keeper) GetEffectiveLeverage(ctx sdk.Context, mtp types.MTP) (math.LegacyDec, error) {
	...
	if mtp.Position == types.Position_LONG {
		...
		custodyInLiabilitiesAsset := mtp.Custody.ToLegacyDec().Mul(tradingAssetPriceDenomRatio)
		...
		liabilitiesInCustodyAsset := mtp.Liabilities.ToLegacyDec().Mul(tradingAssetPriceDenomRatio)
