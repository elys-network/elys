...
func (k Keeper) ProcessOpenLong(ctx sdk.Context, mtp *types.MTP, leverage sdk.Dec, eta sdk.Dec, collateralAmountDec sdk.Dec, poolId uint64, msg *types.MsgOpen, baseCurrency string, isBroker bool) (*types.MTP, error) {
	...
	if mtp.CollateralAsset != baseCurrency {
		...
		borrowingAmount, err := k.EstimateSwapGivenOut(ctx, custodyAmtToken, baseCurrency, ammPool)
		if err != nil {
			return nil, err
		}
	...
	if mtp.CollateralAsset == baseCurrency {
		...
		custodyAmount, err = k.EstimateSwap(ctx, leveragedAmtTokenIn, mtp.CustodyAsset, ammPool)
		if err != nil {
			return nil, err
		}
	}
