...
func (k Keeper) HandleOpenEstimationByFinal(ctx sdk.Context, req *types.QueryOpenEstimationByFinalRequest) (*types.QueryOpenEstimationByFinalResponse, error) {
	...
	if req.Position == types.Position_LONG {
		...
		if mtp.CollateralAsset == baseCurrency {
			...
			collateralLiab, slippage, weightBreakingFee, swapFees, takerFees, err = k.EstimateSwapGivenOut(ctx, req.FinalAmount, mtp.CollateralAsset, ammPool, req.Address, false)
			if err != nil {
				return nil, err
			}
		...
		if mtp.CollateralAsset != baseCurrency {
			...
			mtp.Collateral = collateral
			liabilities, slippage, weightBreakingFee, swapFees, takerFees, err = k.EstimateSwapGivenOut(ctx, sdk.NewCoin(req.FinalAmount.Denom, req.FinalAmount.Amount.Sub(collateral)), baseCurrency, ammPool, req.Address, false)
			if err != nil {
				return nil, err
			}
		}
	...
	if req.Position == types.Position_SHORT {
		...
		liabilities, slippage, weightBreakingFee, swapFees, takerFees, err = k.EstimateSwapGivenIn(ctx, req.FinalAmount, baseCurrency, ammPool, mtp.Address, false)
		if err != nil {
			return nil, err
		}
