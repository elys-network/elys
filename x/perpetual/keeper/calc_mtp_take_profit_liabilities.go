...
func (k Keeper) CalcMTPTakeProfitLiability(ctx sdk.Context, mtp *types.MTP, baseCurrency string) (math.Int, error) {
	...
	if mtp.Position == types.Position_LONG {
		...
		if err != nil {
			return math.ZeroInt(), errorsmod.Wrapf(err, "unable to swap takeProfitCustody (%s) to baseCurrency for takeProfitLiabilities", takeProfitCustody.String())
		}
	} else {
		...
		if err != nil {
			return math.ZeroInt(), errorsmod.Wrapf(err, "unable to swap takeProfitCustody (%s) to LiabilitiesAsset for takeProfitLiabilities", takeProfitCustody.String())
		}
	}
