...
func (_m *OpenChecker) PreparePools(ctx types.Context, collateralAsset string, tradingAsset string) (uint64, ammtypes.Pool, perpetualtypes.Pool, error) {
	...
	if len(ret) == 0 {
		panic("no return value specified for PreparePools")
	}

	var r0 uint64
	var r1 ammtypes.Pool
	var r2 perpetualtypes.Pool
	var r3 error
	if rf, ok := ret.Get(0).(func(types.Context, string, string) (uint64, ammtypes.Pool, perpetualtypes.Pool, error)); ok {
	...
	if rf, ok := ret.Get(2).(func(types.Context, string, string) perpetualtypes.Pool); ok {
		r2 = rf(ctx, collateralAsset, tradingAsset)
	} else {
		r2 = ret.Get(2).(perpetualtypes.Pool)
	}
