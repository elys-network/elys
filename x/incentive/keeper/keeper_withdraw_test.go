...
func TestProcessWithdrawRewards(t *testing.T) {
	...
	// Withdraw rewards
	err = ik.ProcessWithdrawRewards(ctx, addr[0].String(), ptypes.Eden, ctypes.WITHDRAW_MODE_ALL)
	require.NoError(t, err)
	...
	// Withdraw rewards
	err = ik.ProcessWithdrawRewards(ctx, addr[0].String(), ptypes.BaseCurrency, ctypes.WITHDRAW_MODE_ALL)
	require.NoError(t, err)
