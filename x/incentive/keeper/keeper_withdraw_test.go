...
func TestProcessWithdrawRewards(t *testing.T) {
	...
	// Withdraw rewards
	err = ik.ProcessWithdrawRewards(ctx, addr[0].String(), ptypes.Eden, int64(ctypes.EarnType_ALL_PROGRAM))
	require.NoError(t, err)
	...
	// Withdraw rewards
	err = ik.ProcessWithdrawRewards(ctx, addr[0].String(), ptypes.BaseCurrency, int64(ctypes.EarnType_ALL_PROGRAM))
	require.NoError(t, err)
