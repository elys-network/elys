...
func TestProcessWithdrawRewards(t *testing.T) {
	...
	// Withdraw rewards
	err = ik.ProcessWithdrawRewards(ctx, addr[0].String(), ptypes.Eden, ctypes.ALL_PROGRAM)
	require.NoError(t, err)
	...
	// Withdraw rewards
	err = ik.ProcessWithdrawRewards(ctx, addr[0].String(), ptypes.BaseCurrency, ctypes.ALL_PROGRAM)
	require.NoError(t, err)
