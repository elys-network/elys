	...
	if !params.WeightBreakingFeeMultiplier.IsZero() {
		...
		if distanceDiff.IsPositive() {
			if !finalWeightOut.IsZero() && !finalWeightIn.IsZero() && !targetWeightOut.IsZero() && !targetWeightIn.IsZero() {
				weightBreakingFee = params.GetBigDecWeightBreakingFeeMultiplier().Mul(Pow(finalWeightIn.Mul(targetWeightOut).Quo(finalWeightOut).Quo(targetWeightIn), params.GetBigDecWeightBreakingFeeMultiplier()))
			}
		} else {
			if !initialWeightOut.IsZero() && !initialWeightIn.IsZero() && !targetWeightOut.IsZero() && !targetWeightIn.IsZero() {
				weightBreakingFee = params.GetBigDecWeightBreakingFeeMultiplier().
					Mul(Pow(initialWeightOut.Mul(targetWeightIn).Quo(initialWeightIn).Quo(targetWeightOut), params.GetBigDecWeightBreakingFeeMultiplier()))
			}
		}
