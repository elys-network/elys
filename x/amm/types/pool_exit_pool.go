...
func (p *Pool) ExitPool(ctx sdk.Context, oracleKeeper OracleKeeper, accountedPoolKeeper AccountedPoolKeeper, exitingShares math.Int, tokenOutDenom string, params Params, applyWeightBreakingFee bool) (exitingCoins sdk.Coins, weightBalanceBonus math.LegacyDec, slippage math.LegacyDec, swapFee math.LegacyDec, err error) {
	exitingCoins, weightBalanceBonus, slippage, swapFee, err = p.CalcExitPoolCoinsFromShares(ctx, oracleKeeper, accountedPoolKeeper, exitingShares, tokenOutDenom, params, applyWeightBreakingFee)
	if err != nil {
	...
	return exitingCoins, weightBalanceBonus, swapFee, math.LegacyZeroDec(), nil
}
