...
func (suite *TestSuite) TestSwapInAmtGivenOut() {
	for _, tc := range []struct {
		...
		// - Check bonus being zero
		{
			...
			tokenOut:            sdk.NewInt64Coin("uusdc", 100_000_000), // 100 USDC
			inTokenDenom:        "uusdt",
			swapFee:             sdk.ZeroDec(),
			expRecoveryBonus:    sdk.ZeroDec(),
			expTokenIn:          sdk.NewInt64Coin("uusdt", 106191930),
			expErr:              false,
		},
		...
		// - Check bonus not be zero
		{
			...
			swapFee:             sdk.ZeroDec(),
			expRecoveryBonus:    sdk.ZeroDec(),
			expTokenIn:          sdk.NewInt64Coin("uusdt", 134220869),
			expErr:              false,
		},
		...
		// - Check bonus be zero
		{
			...
			inTokenDenom:        "uusdc",
			swapFee:             sdk.ZeroDec(),
			expRecoveryBonus:    sdk.MustNewDecFromStr("0.047742744334716659"),
			expTokenIn:          sdk.NewInt64Coin("uusdc", 93571428),
			expErr:              false,
		},
		...
		// - Check bonus be zero
		{
			...
			expRecoveryBonus:    sdk.ZeroDec(),
			expTokenIn:          sdk.NewInt64Coin("uusdc", 36075037),
			expErr:              false,
		},
	} {
		suite.Run(tc.desc, func() {
			...
			tokenOut, weightBonus, err := pool.SwapInAmtGivenOut(suite.ctx, suite.app.OracleKeeper, sdk.Coins{tc.tokenOut}, tc.inTokenDenom, tc.swapFee, suite.app.AccountedPoolKeeper)
			if tc.expErr {
