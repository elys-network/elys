...
func (suite *TestSuite) TestSwapOutAmtGivenIn() {
	for _, tc := range []struct {
		...
		// - Check bonus being zero
		{
			...
			swapFee:                sdkmath.LegacyZeroDec(),
			expRecoveryBonus:       sdkmath.LegacyMustNewDecFromStr("-0.000330206789333974"),
		...
		// - Check bonus not be zero
		{
			...
			outTokenDenom:          ptypes.BaseCurrency,
			swapFee:                sdkmath.LegacyZeroDec(),
			expRecoveryBonus:       sdkmath.LegacyMustNewDecFromStr("-0.006394728361554381"),
		...
		suite.Run(tc.desc, func() {
			...
			tokenOut, _, _, weightBonus, _, err := pool.SwapOutAmtGivenIn(suite.ctx, suite.app.BankKeeper, sdk.AccAddress{}, suite.app.OracleKeeper, &pool, sdk.Coins{tc.tokenIn}, tc.outTokenDenom, tc.swapFee, suite.app.AccountedPoolKeeper, sdkmath.LegacyOneDec(), params, false)
			if tc.expErr {
				suite.Require().Error(err)
