...
func (suite *TestSuite) TestSwapOutAmtGivenIn() {
	for _, tc := range []struct {
		...
		suite.Run(tc.desc, func() {
			...
			tokenOut, weightBonus, err := pool.SwapOutAmtGivenIn(suite.ctx, suite.app.OracleKeeper, sdk.Coins{tc.tokenIn}, tc.outTokenDenom, tc.swapFee, suite.app.AccountedPoolKeeper)
			if tc.expErr {
				suite.Require().Error(err)
			} else {
				suite.Require().NoError(err)
