...
func (suite *TestSuite) TestSwapOutAmtGivenIn() {
	for _, tc := range []struct {
		...
		// - Check bonus not be zero
		{
			...
			outTokenDenom:          ptypes.BaseCurrency,
			swapFee:                osmomath.ZeroBigDec(),
			expRecoveryBonus:       osmomath.MustNewBigDecFromStr("-0.006347556007845348"),
		...
		// - Check bonus be zero
		{
			...
			outTokenDenom:          "uusdt",
			swapFee:                osmomath.ZeroBigDec(),
			expRecoveryBonus:       osmomath.MustNewBigDecFromStr("0.001558845726811990"),
		...
		suite.Run(tc.desc, func() {
			...
			tokenOut, _, _, weightBonus, _, _, err := pool.SwapOutAmtGivenIn(suite.ctx, suite.app.OracleKeeper, &pool, sdk.Coins{tc.tokenIn}, tc.outTokenDenom, tc.swapFee, suite.app.AccountedPoolKeeper, osmomath.OneBigDec(), params, osmomath.ZeroBigDec())
			if tc.expErr {
				suite.Require().Error(err)
