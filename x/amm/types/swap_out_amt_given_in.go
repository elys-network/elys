...
func GetOraclePoolNormalizedWeights(ctx sdk.Context, poolId uint64, oracleKeeper OracleKeeper, poolAssets []PoolAsset) ([]AssetWeight, error) {
	...
	for _, asset := range poolAssets {
		...
		weight := amount.ToLegacyDec().Mul(tokenPrice).Quo(oraclekeeper.Pow10(tokenDecimal))
...
func (p Pool) CalcGivenInSlippage(
	...
	inTokenPrice, inTokenDecimal := oracleKeeper.GetAssetPriceFromDenom(ctx, tokenIn.Denom)
	if inTokenPrice.IsZero() {
		return sdkmath.LegacyZeroDec(), fmt.Errorf("price for inToken not set: %s", poolAssetIn.Token.Denom)
	}
	outTokenPrice, outTokenDecimal := oracleKeeper.GetAssetPriceFromDenom(ctx, tokenOutDenom)
	if outTokenPrice.IsZero() {
		return sdkmath.LegacyZeroDec(), fmt.Errorf("price for outToken not set: %s", poolAssetOut.Token.Denom)
	}

	oracleOutAmount := sdkmath.LegacyNewDecFromInt(tokenIn.Amount).Mul(inTokenPrice.Mul(oraclekeeper.Pow10(outTokenDecimal))).Quo(outTokenPrice.Mul(oraclekeeper.Pow10(inTokenDecimal)))
