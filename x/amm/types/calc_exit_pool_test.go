...
func TestCalcExitValueWithSlippage(t *testing.T) {
	...
	}{
		{
			...
			types.Pool{
				...
				PoolAssets: []types.PoolAsset{
					{Token: sdk.NewCoin("tokenA", sdkmath.NewInt(1000)), Weight: sdkmath.NewInt(1), ExternalLiquidityRatio: sdkmath.LegacyZeroDec()},
					{Token: sdk.NewCoin("tokenB", sdkmath.NewInt(2000)), Weight: sdkmath.NewInt(1), ExternalLiquidityRatio: sdkmath.LegacyZeroDec()},
				},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			value, _, _, err := types.CalcExitValueWithSlippage(ctx, oracleKeeper, accKeeper, tc.pool, tc.exitingShares, tc.tokenOutDenom, osmomath.ZeroBigDec(), true, types.DefaultParams())
...
func TestCalcExitPool(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			exitCoins, weightBalanceBonus, _, _, _, _, err := types.CalcExitPool(ctx, oracleKeeper, tc.pool, accKeeper, tc.exitingShares, tc.tokenOutDenom, tc.params, osmomath.ZeroBigDec(), true)
			if tc.expectedErrMsg != "" {
				require.Error(t, err)
				require.Contains(t, err.Error(), tc.expectedErrMsg)
			} else {
