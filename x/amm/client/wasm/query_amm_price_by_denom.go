...
func (oq *Querier) queryAmmPriceByDenom(ctx sdk.Context, query *ammtypes.QueryPriceRequest) ([]byte, error) {
	...
	resp, err := oq.keeper.InRouteByDenom(sdk.WrapSDKContext(ctx), &ammtypes.QueryInRouteByDenomRequest{DenomIn: query.Denom, DenomOut: usdcDenom})
	if err != nil {
		return nil, errorsmod.Wrap(err, "failed to get in route by denom")
	}

	routes := resp.InRoute
	tokenIn := sdk.NewCoin(query.Denom, sdk.NewInt(1000000))
	...
	// Divide by 1 million
	spotPrice = spotPrice.QuoInt(sdk.NewInt(1000000))
