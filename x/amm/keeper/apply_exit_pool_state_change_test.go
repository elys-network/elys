	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				err := suite.app.AmmKeeper.ApplyExitPoolStateChange(suite.ctx, pool, addr, pool.TotalShares.Amount, coins, false, sdkmath.LegacyZeroDec(), sdkmath.LegacyZeroDec())
		...
		},
		{
			...
			func() {
				...
				ctx = ctx.WithBlockTime(ctx.BlockTime().Add(time.Hour))
				err = app.AmmKeeper.ApplyExitPoolStateChange(ctx, pool, addr, pool.TotalShares.Amount, coins, false, sdkmath.LegacyZeroDec(), sdkmath.LegacyZeroDec())
				suite.Require().Error(err)
			},
			func() {},
		},
