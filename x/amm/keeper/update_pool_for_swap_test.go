...
func (suite *AmmKeeperTestSuite) TestUpdatePoolForSwap() {
	for _, tc := range []struct {
		...
		suite.Run(tc.desc, func() {
			...
			err = suite.app.AmmKeeper.UpdatePoolForSwap(suite.ctx, pool, sender, sender, tc.tokenIn, tc.tokenOut, tc.swapFee, sdkmath.LegacyZeroDec(), tc.tokenIn.Amount, tc.tokenOut.Amount, tc.weightBalanceBonus, false)
			if !tc.expPass {
