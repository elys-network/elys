...
func (k Keeper) ExitPool(
...
) (exitCoins sdk.Coins, weightBalanceBonus math.LegacyDec, swapFee math.LegacyDec, err error) {
	...
	exitCoins, weightBalanceBonus, swapFee, err = pool.ExitPool(ctx, k.oracleKeeper, k.accountedPoolKeeper, shareInAmount, tokenOutDenom, params, applyWeightBreakingFee)
	if err != nil {
	...
	if !tokenOutMins.DenomsSubsetOf(exitCoins) || tokenOutMins.IsAnyGT(exitCoins) {
		return sdk.Coins{}, math.LegacyZeroDec(), math.LegacyZeroDec(), errorsmod.Wrapf(types.ErrLimitMinAmount,
	...
	return exitCoins, weightBalanceBonus, swapFee, nil
}
