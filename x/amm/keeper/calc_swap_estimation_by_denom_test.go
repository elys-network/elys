...
func TestCalcSwapEstimationByDenom(t *testing.T) {
	...
	inRoute, outRoute, tokenOut, spotPrice, _, _, _, _, err := k.CalcSwapEstimationByDenom(ctx, amount, "denom1", "denom2", "baseCurrency", sdk.ZeroDec(), sdk.ZeroDec())
	...
	_, _, _, _, _, _, _, _, err = k.CalcSwapEstimationByDenom(ctx, amount, "denom1", "denom2", "baseCurrency", sdk.ZeroDec(), sdk.ZeroDec())
	require.Error(t, err)
}
