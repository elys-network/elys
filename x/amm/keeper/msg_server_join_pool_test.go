...
func (suite *AmmKeeperTestSuite) TestMsgServerJoinPool() {
	for _, tc := range []struct {
		...
		},
		{
			...
			shareOutAmount:   math.NewInt(832154822031355754), // weight breaking fee - slippage enable
			expSenderBalance: sdk.Coins{},
		...
		},
		{
			...
			shareOutAmount:   math.NewInt(644790550160842642), // weight recovery direction - slippage enable
			expSenderBalance: sdk.Coins{},
			expTokenIn:       sdk.Coins{sdk.NewInt64Coin("uusdt", 1000000)},
			expPass:          true,
		},
...
func (suite *AmmKeeperTestSuite) TestMsgServerJoinPoolExploitScenario() {
	for _, tc := range []struct {
		...
		suite.Run(tc.desc, func() {
			...
			joinValueWithSlippage, _, _, _ := pool.CalcJoinValueWithSlippage(suite.ctx, snapshot, suite.app.OracleKeeper, tc.senderInitBalance[0], initialWeightOut, types.DefaultParams())
