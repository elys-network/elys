...
func (suite *AmmKeeperTestSuite) TestMsgServerJoinPool() {
	for _, tc := range []struct {
		...
		},
		{
			...
			shareOutAmount:   math.NewInt(832154489169426941), // weight breaking fee - slippage enable
			expSenderBalance: sdk.Coins{},
...
func (suite *AmmKeeperTestSuite) TestMsgServerJoinPoolExploitScenario() {
	for _, tc := range []struct {
		...
		suite.Run(tc.desc, func() {
			...
			joinValueWithSlippage, _, _ := pool.CalcJoinValueWithSlippage(suite.ctx, suite.app.OracleKeeper, suite.app.AccountedPoolKeeper, tc.senderInitBalance[0], initialWeightOut, types.DefaultParams())
			tvl, _ := pool.TVL(suite.ctx, suite.app.OracleKeeper, suite.app.AccountedPoolKeeper)
			expectedNumShares := joinValueWithSlippage.
				MulInt(totalShares).Quo(tvl).ToInt()
