...
func (k Keeper) JoinPoolNoSwap(
	...
	if !pool.PoolParams.UseOracle {
		...
		tokensJoined, sharesOut, slippage, weightBalanceBonus, _, weightMultiplier, err := pool.JoinPool(ctx, &snapshot, k.oracleKeeper, k.accountedPoolKeeper, tokensIn, params, takerFees)
		if err != nil {
			return nil, sdkmath.ZeroInt(), err
		}
		...
		err = k.ApplyJoinPoolStateChange(ctx, pool, sender, sharesOut, tokensJoined, weightBalanceBonus, weightMultiplier)
		if err != nil {
	...
	snapshot := k.GetAccountedPoolSnapshotOrSet(ctx, pool)
	tokensJoined, sharesOut, _, weightBalanceBonus, _, weightMultiplier, err := pool.JoinPool(ctx, &snapshot, k.oracleKeeper, k.accountedPoolKeeper, tokenInMaxs, params, takerFees)
	if err != nil {
		return nil, sdkmath.ZeroInt(), err
	}
