...
func (suite *AmmKeeperTestSuite) TestCalcInAmtGivenOut() {
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				_, _, _, err := suite.app.AmmKeeper.CalcInAmtGivenOut(suite.ctx, 0, suite.app.OracleKeeper, ammtypes.SnapshotPool{}, sdk.Coins{}, "", osmomath.ZeroBigDec())
		...
		},
		{
			...
			func() {
				...
				_, _, _, err := suite.app.AmmKeeper.CalcInAmtGivenOut(suite.ctx, pool.PoolId, suite.app.OracleKeeper, snapshot, sdk.NewCoins(sdk.NewCoin(ptypes.BaseCurrency, math.NewInt(10000))), ptypes.ATOM, osmomath.ZeroBigDec())
