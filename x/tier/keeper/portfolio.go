...
func (k Keeper) RetrieveLeverageLpTotal(ctx sdk.Context, user sdk.AccAddress) (sdk.Dec, sdk.Dec, sdk.Dec) {
	...
	if err == nil {
		for _, position := range positions {
			pool, found := k.amm.GetPool(ctx, position.Position.Position.AmmPoolId)
			if !found {
				continue
			}
			info := k.amm.PoolExtraInfo(ctx, pool)
			amount := position.Position.Position.LeveragedLpAmount.ToLegacyDec()
			totalValue = totalValue.Add(amount.Mul(info.LpTokenPrice).QuoInt(ammtypes.OneShare))
			// USD value of debt
			debt := k.stablestakeKeeper.GetDebt(ctx, position.Position.GetPosition().GetPositionAddress())
			usdcDenom, found := k.assetProfileKeeper.GetUsdcDenom(ctx)
			if !found {
				continue
			}
