	...
	for _, user := range listCommitments {
		...
		for _, commitment := range user.CommittedTokens {
			if strings.HasPrefix(commitment.Denom, "stablestake/share") {
				...
				tokenPrice, _ := k.oracleKeeper.GetAssetPriceFromDenom(ctx, borrowPool.GetDepositDenom())
				fiatValue := tokenPrice.MulInt(commitment.Amount).MulLegacyDec(redemptionRate)
