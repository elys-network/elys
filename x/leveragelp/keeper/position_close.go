...
func (k Keeper) CheckHealthStopLossThenRepayAndClose(ctx sdk.Context, position *types.Position, pool *types.Pool, closingRatio math.LegacyDec, isLiquidation bool) (math.LegacyDec, math.Int, sdk.Coins, math.Int, sdk.Coins, elystypes.Dec34, bool, elystypes.Dec34, elystypes.Dec34, elystypes.Dec34, elystypes.Dec34, error) {

	positionHealth, err := k.GetPositionHealth(ctx, *position)
	if err != nil {
		return math.LegacyZeroDec(), math.ZeroInt(), sdk.Coins{}, math.ZeroInt(), sdk.Coins{}, elystypes.ZeroDec34(), false, elystypes.ZeroDec34(), elystypes.ZeroDec34(), elystypes.ZeroDec34(), elystypes.ZeroDec34(), err
	}
	...
	if !found {
		return math.LegacyZeroDec(), math.ZeroInt(), sdk.Coins{}, math.ZeroInt(), sdk.Coins{}, elystypes.ZeroDec34(), false, elystypes.ZeroDec34(), elystypes.ZeroDec34(), elystypes.ZeroDec34(), elystypes.ZeroDec34(), types.ErrAmmPoolNotFound
	}
	...
	if closingRatio.IsNil() || closingRatio.IsNegative() || closingRatio.IsZero() || closingRatio.GT(math.LegacyOneDec()) {
		return math.LegacyZeroDec(), math.ZeroInt(), sdk.Coins{}, math.ZeroInt(), sdk.Coins{}, elystypes.ZeroDec34(), stopLossReached, elystypes.ZeroDec34(), elystypes.ZeroDec34(), elystypes.ZeroDec34(), elystypes.ZeroDec34(), errors.New("invalid closing ratio for leverage lp")
	}
	...
	if err != nil {
		return math.LegacyZeroDec(), math.ZeroInt(), sdk.Coins{}, math.ZeroInt(), sdk.Coins{}, elystypes.ZeroDec34(), stopLossReached, elystypes.ZeroDec34(), elystypes.ZeroDec34(), elystypes.ZeroDec34(), elystypes.ZeroDec34(), err
	}
	collateralDenomPrice, _ := k.oracleKeeper.GetAssetPriceFromDenom(ctx, position.Collateral.Denom)
	...
	lpSharesForRepay := repayValue.MulInt(ammPool.TotalShares.Amount).Quo(ammPoolTVL).Ceil().ToInt() // round up
	...
	exitFeeOnClosingPosition := elystypes.ZeroDec34()
	percentageExitLeverageFee := elystypes.ZeroDec34()
	weightBreakingFee := elystypes.ZeroDec34()
	weightBreakingFeeValue := elystypes.ZeroDec34()
	...
	if weightBalanceBonus.IsNegative() {
		...
		exitFeeOnClosingPosition = weightBreakingFeeValue.MulInt(ammPool.TotalShares.Amount).Quo(ammPoolTVL.MulInt(totalLpAmountToClose))
	}

	slippageValue := slippage.Mul(repayValue)
	swapFeeValue := swapFee.Mul(repayValue)
	takerFeeValue := takerFee.Mul(repayValue)
	exitSlippageFeeOnClosingPosition := (slippageValue).MulInt(ammPool.TotalShares.Amount).Quo(ammPoolTVL.MulInt(totalLpAmountToClose))
	exitSwapFeeOnClosingPosition := (swapFeeValue).MulInt(ammPool.TotalShares.Amount).Quo(ammPoolTVL.MulInt(totalLpAmountToClose))
	exitTakerFeeOnClosingPosition := (takerFeeValue).MulInt(ammPool.TotalShares.Amount).Quo(ammPoolTVL.MulInt(totalLpAmountToClose))
	...
	if totalLpAmountToClose.GT(lpSharesForRepay) {
		...
		denominator := ammPoolTVL.MulInt(totalLpAmountToClose.Sub(lpSharesForRepay)).QuoInt(ammPool.TotalShares.Amount)
		percentageExitLeverageFee = (weightBreakingFeeValue.Add(slippageValue).Add(swapFeeValue).Add(takerFeeValue)).Quo(denominator)
	}

	if percentageExitLeverageFee.GT(elystypes.OneDec34()) {
		percentageExitLeverageFee = elystypes.OneDec34()
	}
	...
	if percentageExitLeverageFee.IsPositive() && len(coinsLeftAfterRepay) > 0 {
		...
		coinsToAmmRebalancer := ammkeeper.PortionCoins(coinsForAmm, elystypes.NewDec34FromLegacyDec(weightBreakingFeePortion))
