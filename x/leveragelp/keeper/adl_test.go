...
func (suite *KeeperTestSuite) TestClosePositionsOnADL() {
	...
	err = suite.app.LeveragelpKeeper.ClosePositionsOnADL(suite.ctx, leveragePool, currentLeverageRatio)
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestClosePositionsOnADL_NegativeClosingRatio() {
	suite.ResetSuite()
	suite.SetupCoinPrices(suite.ctx)
	addresses := simapp.AddTestAddrs(suite.app, suite.ctx, 5, sdkmath.NewInt(1000000))
	asset1 := ptypes.ATOM
	...
	exitPool := ammtypes.MsgExitPool{
		...
		PoolId:        1,
		MinAmountsOut: sdk.NewCoins(sdk.NewInt64Coin(ptypes.ATOM, 1), sdk.NewInt64Coin(ptypes.BaseCurrency, 1)),
		ShareAmountIn: sdkmath.LegacyMustNewDecFromStr("1000000000000000000000").TruncateInt(),
		TokenOutDenom: "",
	}
	...
	err = suite.app.LeveragelpKeeper.ClosePositionsOnADL(suite.ctx, leveragePool, currentLeverageRatio)
	suite.Require().Error(err, errors.New("closing ratio is <= 0 for pool while triggering adl 1"))
