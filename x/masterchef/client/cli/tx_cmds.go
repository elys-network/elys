...
func CmdUpdateIncentiveParams() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update-incentive-params [reward-portion-for-lps] [max-eden-reward-apr-lps]",
		Short: "Broadcast message update-incentive-params [reward-portion-for-lps] [max-eden-reward-apr-lps]",
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			rewardPortionForLps := sdk.MustNewDecFromStr(args[0])
			maxEdenRewardAprLps := sdk.MustNewDecFromStr(args[1])

			govAddress := sdk.AccAddress(address.Module("gov"))
			msg := &types.MsgUpdateIncentiveParams{
				Authority:           govAddress.String(),
