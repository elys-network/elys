...
func (k Keeper) ConvertGasFeesToUsdc(ctx sdk.Context, baseCurrency string, address sdk.AccAddress) (sdk.Coins, error) {
	...
	for _, tokenIn := range feesCollected {
		...
		err = k.amm.UpdatePoolForSwap(ctx, pool, address, address, tokenIn, tokenOutCoin, math.LegacyZeroDec(), math.ZeroInt(), oracleOutAmount.TruncateInt(), math.LegacyZeroDec(), false)
		if err != nil {
			return sdk.Coins{}, err
		}
